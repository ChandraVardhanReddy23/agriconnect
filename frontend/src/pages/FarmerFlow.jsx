...
export default function FarmerFlow({ user, onLogout }) {
  ...
  const [form, setForm] = useState({ crop: '', variety: '', quantity: '', unit: 'quintal', price: '', location: user.location || '', harvest_date: '', quality: 'standard', description: '', farmer_id: user.id });
  ...
  async function refresh() {
    ...
    if (nextOrders[0]?.buyer_id) {
      try { setLogistics(await api(`/logistics/optimize-routes?buyer_id=${nextOrders[0].buyer_id}`)); } catch { setLogistics(null); }
    } else setLogistics(null);
      ...
      <div className="grid two">
        <section className="card"><h3>List new produce</h3><div className="steps"><span className={step >= 1 ? 'on' : ''}>1 Crop</span><span className={step >= 2 ? 'on' : ''}>2 Details</span><span className={step >= 3 ? 'on' : ''}>3 Publish</span></div>
          <form onSubmit={step < 3 ? (event) => { event.preventDefault(); setStep(step + 1); } : publish}>
            ...
            {step === 2 && <><div className="form-row"><label>Quantity<input name="quantity" type="number" min="1" value={form.quantity} onChange={set} required /></label><label>Unit<select name="unit" value={form.unit} onChange={set}><option>quintal</option><option>kg</option><option>tonne</option></select></label></div><label>Expected price<input name="price" type="number" min="1" value={form.price} onChange={set} required /></label><label>Location<input name="location" value={form.location} onChange={set} /></label></>}
