...
def public_user(user: dict) -> dict:
    return {key: user[key] for key in (
        "id", "name", "email", "role", "location", "phone", "rating",
...
def signup(payload: SignupRequest) -> dict:
    with get_connection() as conn:
        try:
            cursor = conn.execute(
                """INSERT INTO users
                   (name,email,password,role,location,phone,vehicle_type,
                    vehicle_capacity,lat,lon)
                   VALUES (?,?,?,?,?,?,?,?,?,?)""",
                (payload.name, payload.email.lower().strip(), payload.password, payload.role,
                 payload.location, payload.phone, payload.vehicle_type,
                 payload.vehicle_capacity, payload.lat, payload.lon),
            )
